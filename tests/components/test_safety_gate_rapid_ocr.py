...
def test_compact_strips_separators_and_uppercases():
...
def test_edit_distance_abandons_early_on_length():
...
def test_device_signatures_reads_model_and_make():
...
def test_device_signatures_drops_signatures_too_short_to_be_evidence():
...
def test_is_device_watermark_tolerates_misreads_but_not_real_product_text():
...
def test_no_signature_disables_the_rule():
    pairs = [("Arcobaleno", 0.9), ("OPPO A98 5G", 1.0)]
    assert suppress_watermarks(pairs, []) == (pairs, 0)


def test_suppression_is_restricted_to_the_tail():
...
def test_trailing_timestamp_goes_only_with_a_watermark():
...
def test_frontmatter_carries_the_count_but_never_the_string():
...
def test_prefixed_name_and_sort_order():
...
def test_reliability_bands_and_empty_image():
...
def test_normalise_result_handles_both_engine_shapes():
    class V2:
        txts = ("a", "b")
        scores = (0.9, 0.8)
